...
async def _async_store_from_database(db_config) -> AsyncIterator[BaseStore]:
    """Construct a Store from the unified ``database:`` configuration."""
    ...
    if db_config.backend == "sqlite":
        ...
        async with AsyncSqliteStore.from_conn_string(conn_str) as store:
            await store.setup()
            logger.info("Store: using AsyncSqliteStore from database config (%s)", conn_str)
            yield store
        return
    ...
    if db_config.backend == "postgres":
        ...
        async with AsyncPostgresStore.from_conn_string(db_config.postgres_url) as store:
            await store.setup()
            logger.info("Store: using AsyncPostgresStore from database config")
            yield store
        return
